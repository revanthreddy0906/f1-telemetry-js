...
import { processSeriesData, type ProcessSeriesInput, type ProcessSeriesOutput } from "./processing";
...
interface WorkerProcessMessage {
  ok: boolean;
  result?: ProcessSeriesOutput<Record<string, number[]>>;
  error?: string;
}
...
const processOnMainThread = <T extends Record<string, number[]>>(
  payload: WorkerProcessingPayload
): ProcessSeriesOutput<T> =>
  processSeriesData<T>({
    context: payload.context,
    time: payload.time,
    seriesMap: payload.seriesMap,
    processing: payload.processing
  } satisfies ProcessSeriesInput);
 ...
 */
export const processSeriesDataInWorker = async <T extends Record<string, number[]>>(
  payload: WorkerProcessingPayload,
  options: WorkerProcessingOptions = {}
): Promise<ProcessSeriesOutput<T>> => {
  if (!canUseWorker()) {
    return processOnMainThread<T>(payload);
  }

  const workerUrl = URL.createObjectURL(new Blob([WORKER_SOURCE], { type: "text/javascript" }));
  const worker = new Worker(workerUrl);
  const timeoutMs = typeof options.timeoutMs === "number" && options.timeoutMs > 0 ? options.timeoutMs : 8_000;

  try {
    const result = await new Promise<ProcessSeriesOutput<T>>((resolve, reject) => {
      const timeoutId = globalThis.setTimeout(() => {
        reject(new Error("worker-timeout"));
      }, timeoutMs);

      worker.onmessage = (event: MessageEvent<WorkerProcessMessage>) => {
        globalThis.clearTimeout(timeoutId);
        if (!event.data?.ok || !event.data.result) {
          reject(new Error(event.data?.error ?? "worker-processing-failed"));
          return;
        }
        resolve(event.data.result as ProcessSeriesOutput<T>);
      };
